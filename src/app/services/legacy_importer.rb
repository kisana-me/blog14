class LegacyImporter
  DEFAULT_DIR = Rails.root.join("storage", "ivecolor-exported").freeze
  ...
  post.content = transform_legacy_images_in_markdown(raw_content)
    ...
    if comments_path.file?
      import_comments_from_json(post, comments_path)
    end
  rescue => e
  ...
  def import_comments_from_json(post, path)
    ...
    arr.each do |c|
      ...
      comment = Comment.new()
      comment.post = post
      if c["account_aid"].present?
        comment.account = Account.find_by(aid: c["account_aid"].to_s[0, 14])
      end
  ...
  def parse_time(str)
    ...
    Time.zone.parse(str) rescue Time.parse(str)
  end
  ...
  def transform_legacy_images_in_markdown(text)
    ...
    text.gsub(/!\[(?<alt>[^\]]*)\]\((?<inner>[^)]+)\)/) do |match|
      ...
      unless url.start_with?(domain_prefix)
        match # keep original
      else
        begin
